...
import { BookOpen, Users, WalletCards, ClipboardCheck, ArrowRight } from 'lucide-react';
...
export const FeatureCards: React.FC<FeatureCardsProps> = ({ onNavigate }) => {
  const features: FeatureCardItem[] = [
    {
      ...
      subtitle: 'Curriculum, classes, subjects and timetable operations.',
      targetHash: 'academic-management',
    ...
    },
    {
      ...
      subtitle: 'Admissions, profiles, attendance and student records.',
      targetHash: 'student-management',
    ...
    },
    {
      ...
      subtitle: 'Staff, payroll, fee collection and financial ledgers.',
      targetHash: 'hr-finance-management',
      icon: <WalletCards className="w-6 h-6 text-purple-600" />,
    ...
    },
    {
      ...
      subtitle: 'Exam scheduling, marks, grading and report cards.',
      targetHash: 'exam-management',
  ...
  const handleCardClick = (targetHash: string) => {
    const route = `/services#${targetHash}`;
    if (onNavigate) {
      onNavigate(route);
    } else {
      ...
      <div className="ingo-feature-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
        {features.map((card, index) => (
          <motion.div
            ...
            onClick={() => handleCardClick(card.targetHash)}
